...
def addDiagnostico():
    ...
    try:
        ...
        diagnostico_id = diagdao.guardarDiagnostico(nombre, descripcion)

        if diagnostico_id is not None:
            return jsonify({
                'success': True,
                'data': {'id': diagnostico_id, 'nombre': nombre, 'descripcion': descripcion},
                'error': None
            }), 201
        else:
            return jsonify({
...
def updateDiagnostico(diagnostico_id):
    ...
    try:
        if diagdao.updateDiagnostico(diagnostico_id, nombre, descripcion):
            return jsonify({
                'success': True,
                'data': {'id': diagnostico_id, 'nombre': nombre, 'descripcion': descripcion},
                'error': None
